...
- **Visit detail popup** - a visit is a TRIP, so clicking one on either detail page opens the trip's own facts (logged-by rep, notes, duration, next-visit date) plus every person/category met on it, each expandable in place for their own outcome, contact snapshot, and whether they asked for something, with its own **✕** to remove just that person from the trip. An Edit button opens the same visit-logging form (pre-filled with everyone already on file), and a bottom-left **Delete** removes the whole trip, everyone on it included. Keeps the inline visit-history rows themselves down to just date + who/where + a summary of who was met.
...
- **visits** - one planned/completed/skipped TRIP by a user to a place on a date, with `sort_order` (route order), notes, `next_visit_date`, `actual_duration_minutes`, `visit_type`, and a `place_name` snapshot so history stays readable even after the live place record is gone. `place_id` is `ON DELETE SET NULL`. Restructured 2026-08-05 (see `HANDOFF.md` §17) - who was met moved out to `visit_encounters` below.
- **visit_encounters** - one row per person/category met on a trip (`visit_id` `ON DELETE CASCADE`, `person_id` `ON DELETE SET NULL`, `person_name/title/email/phone` snapshot, `met_with_type, outcome, they_requested`). A trip where a rep met a named contact AND got gatekept by the front desk is one `visits` row with two encounters; a still-planned trip has none yet. A partial unique index stops the same person being recorded twice on one trip.
