...
const SECTIONS = [
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'driveTime.MIN_DRIVE_MINUTES', label: 'Minimum drive between stops', unit: 'min', type: 'integer', min: 0, max: 60,
            help: 'A floor under an ESTIMATED drive time, so an uncached pair never lands at nearly zero minutes. Only applies when the real routing service has no cached answer for a leg - a real cached duration (including a genuine near-zero one, like two suites in the same building) is shown as-is.',
            raise: 'An uncached leg is budgeted more conservatively.',
            lower: 'An uncached leg can budget down toward the raw straight-line estimate.' },
        ],
      },
    ],
  },
  ...
  // ---------------------------------------------------------- route planner
  {
    ...
    icon: '🗺️',
    blurb:
      'Bounds on what you can ask the planner for, and the settings behind how it sequences a day\'s stops from the cached distance matrix.',
    groups: [
      {
        id: 'plan-bounds',
      ...
      },
      {
        ...
        blurb: 'The app sequences a day\'s stops locally, from a cache of real road distances - no live call, no network dependency, so this never blocks planning.',
        fields: [
      ...
      {
        id: 'plan-matrix-fallback',
        title: 'Distance Cache Fallback',
        blurb: 'Distances come from a cache of real, previously-computed road distances. A place pair not yet in that cache (a brand-new place, or one still waiting on the backfill queue) uses a straight-line estimate instead, calibrated to real Lincoln, NE road distances - this is a different estimate from the Drive Time Estimates section above, used only for this specific gap.',
        fields: [
          { key: 'matrixCache.DETOUR_FACTOR', label: 'Fallback distance multiplier', unit: 'x', type: 'number', min: 0.5, max: 3, step: 0.01,
            help: 'Scales the straight-line (grid) distance used only for a place pair missing from the cache. 1.0 means trust the raw grid distance as-is. Calibrated from a real sample of Lincoln, NE road distances - recalibrate the same way once there\'s a fuller backfill to sample from.',
            raise: 'Uncached pairs are estimated as farther apart.', lower: 'Uncached pairs are estimated as closer together.' },
          { key: 'matrixCache.FALLBACK_SPEED_MPH', label: 'Fallback average speed', unit: 'mph', type: 'number', min: 1, max: 80, step: 1,
            help: 'Flat average speed used to turn a fallback distance into a fallback drive time, for a place pair missing from the cache.',
            raise: 'Uncached legs are estimated as faster (less drive time).',
            lower: 'Uncached legs are estimated as slower (more drive time).' },
        ],
      },
    ],
  },
...
const NAMESPACES = {
  ...
  matrixCache: require('./matrixCache'),
